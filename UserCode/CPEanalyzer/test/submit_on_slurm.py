...
#config.sbatch_output = 'slurm/slurm-%j.out'
...
# Defaults to an empty string.
#config.sbatch_error = 'slurm/slurm-%j.out'
...
# Defaults to an empty string.
config.cmsswDir = '/home/ucl/cp3/cdelaere/scratch/sandbox/CMSSW_10_6_2/src'
...
# Only needed if 'stageout' is True.
# Defaults to the directory defined in 'sbatch_workdir'.
config.stageoutDir = config.sbatch_workdir + '/slurm'
...
# Defaults to an empty list (i.e. no input parameters).
#config.inputParams = []
with open("allFiles_run320413.txt") as file:
  files = file.readlines()
files = [file.rstrip('\n') for file in files]
files = [file[17:] for file in files]
filesPerJob = 20
sliced = [files[i:i+filesPerJob] for i in range(0,len(files),filesPerJob)]
config.inputParams = [ [ ",".join(input), "output_%d.root"%idx ] for idx,input in enumerate(sliced) ]
